...
def call_sheets_api(service, spreadsheet_id):
    ...
    for i in range(100):
        try:
            ...
            time.sleep(3)
            continue
        raise Exception("FAILED TO CALL_SHEETS_API")
...
def calculate_case(patient_id):
    ...
    if last_case_query_result is not None:
        last_case_id = last_case_query_result[0]
    else:
        last_case_id = 0
...
def add_requisition(barcode_subfolder_id, sheets_service, drive_service, surname):
    ...
    spreadsheet_id = create_spreadsheet_in_barcode(barcode_subfolder_id, drive_service, surname)
...
def add_sheet_to_requisition(service, sheet_id):
    ...
    for i in range(100):
        try:
            ...
            time.sleep(3)
            continue
        raise Exception("FAILED TO UPDATE GOOGLE")
    print(res)
...
def copy_from_main_to_requisition(service, spreadsheet_id, template_id):
    ...
    for i in range(100):
        try:
            ...
            time.sleep(3)
            continue
        raise Exception("FAILED TO COPY FROM MAIN TO REQUISITION")
    return res['sheetId']
...
def rename_sheet(service, spreadsheet_id, new_name, sheet_id):
    ...
    for i in range(100):
        try:
            ...
            time.sleep(3)
            continue
        raise Exception("FAILED TO RENAME_SHEET")
    return res
...
print(f"STARTED {now}", file=sys.stderr)
for number, line in enumerate(values):
    ...
    if (line[0] == 'add' or line[0] == 'add-force'):
        ...
        print(patient_id)
        correct_line = True
        patient_subfolder_id = AODDB.get_Patient_folder(patient_id)
    if patient_id is not None and correct_line is True:
        ...
        if (str(line[7]).upper() == 'CDX'):
            requisition_id = add_requisition(barcode_subfolder_id, sheets_service, drive_service, line[2])
